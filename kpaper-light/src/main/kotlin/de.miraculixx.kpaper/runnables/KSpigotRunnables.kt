...
internal object KRunnableHolder : AutoCloseable {
    ...
    fun add(runnable: BukkitRunnable, callback: () -> Unit, safe: Boolean) = runnableEndCallbacks.put(runnable, Pair(callback, safe))
...
fun task(
    ...
    val bukkitRunnable = object : KSpigotRunnable() {
        ...
        override fun run() {
            ...
            if (howOften != null) {
                ...
                curCounter++
                if (curCounter >= howOften) ranOut = true
            ...
            if (isCancelled) {
                if (safe || ranOut) KRunnableHolder.activate(this)
                else KRunnableHolder.remove(this)
            }
        }
    }
    ...
    if (period != null) if (sync) bukkitRunnable.runTaskTimer(PluginInstance, delay, period)
    else bukkitRunnable.runTaskTimerAsynchronously(PluginInstance, delay, period)
    else if (sync) bukkitRunnable.runTaskLater(PluginInstance, delay)
