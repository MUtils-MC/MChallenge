...
class StorageGUI(
    ...
    override val defaultClickAction: ((InventoryClickEvent) -> Unit) = action@{
        ...
        when (item?.itemMeta?.customModel) {
            ...
            9001 -> {
                ...
                else 1).coerceAtMost(content.size / 9 - 3)
                player.click()
                build()
            }
            9002 -> {
    ...
    private fun build() {
        ...
        if (scrollable) {
            ...
            i.setItem(8, if (page >= content.size / 9 - 3) arrowDownRed else arrowDownGreen)
        }
        ...
        visible.forEachIndexed { index, pair ->
            if (pair.second) {
                ...
                pair.first.addEnchantment(Enchantment.MENDING, 1)
    ...
    private fun fillPlaceholder() {
        ...
        val lightHolder = itemStack(Material.LIGHT_GRAY_STAINED_GLASS_PANE) { meta { displayName(emptyComponent()) } }
