...
class MobRandomizerData(private val random: Boolean) {
    ...
    fun generate() {
        ...
        for (material in Material.values()) {
            blockList[material] = list[random.nextInt(list.size)]
        }
    }
    ...
    fun spawnEntity(block: Block) {
        ...
        if (entity is LivingEntity) {
            if (entity.type == EntityType.RABBIT) {
                (entity as Rabbit).rabbitType = Rabbit.Type.THE_KILLER_BUNNY
            }

            if (block.type == Material.AIR || block.type == Material.FIRE) return
        ...
        when (entity.type) {
            EntityType.FIREBALL -> (entity as Fireball).yield = 1F
            EntityType.PRIMED_TNT -> (entity as TNTPrimed).fuseTicks = 30
            else -> {}
        }
    }
}