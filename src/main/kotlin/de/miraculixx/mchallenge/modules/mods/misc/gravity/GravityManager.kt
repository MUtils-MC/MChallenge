...
class GravityManager : Challenge {
    ...
    private fun run() {
        ...
        task(false, 0, 20) {
            ...
            if (current == GravityState.DEFAULT && counter <= 10) {
                bar.name(dominantLocale.msg("event.gravity.switch", listOf(counter.toString())))
    ...
    private fun newGravity(last: GravityState): Pair<GravityState, Gravity?> {
        ...
        if (debug) broadcast( cmp("- Start ${newGravity.name}"))
        sync { gravityClass?.start() ?: broadcast(prefix + cmp("Unexpected Error: ClassNotFound", cError)) }
