...
class NoSameItem : Challenge {
    ...
    private fun getBossBar(uuid: UUID): BossBar {
        val locale = Bukkit.getPlayer(uuid)?.language() ?: defaultLocale
        return barMap.getOrPut(uuid) {
            BossBar.bossBar(locale.msg("event.noSameItem.deathTitle"), 1f, BossBar.Color.RED, BossBar.Overlay.PROGRESS)
        }
    }
    ...
    private fun addItems(player: Player, items: Set<Material>, type: String) {
        ...
        items.forEach { item ->
            ...
            if (order.isNotEmpty()) { //Another player already collected it
                ...
                player.sendMessage(prefix + player.msg("event.noSameItem.duplicate", listOf(firstPlayer?.name ?: "Unknown", item.name)))
            }
            order.add(uuid)
    ...
    private fun updateBossBar(uuid: UUID) {
        ...
        val name = if (hearts <= 0) {
            val locale = Bukkit.getPlayer(uuid)?.language() ?: defaultLocale
            locale.msg("event.noSameItem.deathTitle")
        } else {
