...
import de.miraculixx.kpaper.extensions.bukkit.msgString
...
class LimitedSkills : Challenge {
    ...
    override fun start(): Boolean {
        if (randomRoles) {
            ...
            onlinePlayers.forEach {
                GUITypes.CH_LIMITED_SKILLS.buildInventory(it, "LIMITED_SKILLS", LimitSkillsItems(this, it.language()), LimitGUI(this))
            }
        }
        return true
    }
    ...
    fun startGame() {
        ...
        playerNoSee.forEach { player ->
            ...
            player.showTitle(Title.title(cmp(player.msgString("items.event.LIMITED_DAMAGE.n"), cHighlight), emptyComponent(), Title.Times.times(Duration.ofMillis(500), times, times)))
            player.world.entities.forEach { e -> player.hideEntity(PluginManager, e) }
        }
        playerSee.forEach { player ->
            player.closeInventory()
            val times = Duration.ofSeconds(3)
            player.showTitle(Title.title(cmp(player.msgString("items.event.LIMITED_SEE.n"), cHighlight), emptyComponent(), Title.Times.times(Duration.ofMillis(500), times, times)))
        }
    }
