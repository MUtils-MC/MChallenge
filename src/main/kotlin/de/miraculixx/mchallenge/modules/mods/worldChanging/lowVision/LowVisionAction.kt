...
class LowVisionAction(private val maxSelects: Int, private val player: Player, private val mWebAPI: MWebAPI) : GUIEvent, ResourcePackChallenge {
    ...
    override val run: (InventoryClickEvent, CustomInventory) -> Unit = event@{ it: InventoryClickEvent, inv: CustomInventory ->
        ...
        when (meta.customModel) {
            10 -> {
                AwaitChatMessage(player, "Filter", 30, null, false, player.msg("event.enterFilter", listOf("blocks")), {
    ...
    private fun createResourcePack(selected: Set<Material>) {
        ...
        File(
            blockModelFolder,
            "transparent_all.json"
        ).writeText("{\"parent\":\"block/transparent\",\"textures\":{\"particle\":\"#all\",\"down\":\"#all\",\"up\":\"#all\",\"north\":\"#all\",\"east\":\"#all\",\"south\":\"#all\",\"west\":\"#all\"}}")
