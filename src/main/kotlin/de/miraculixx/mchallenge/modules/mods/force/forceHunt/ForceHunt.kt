...
class ForceHunt : Challenge {
    private val dominantLocale = if (onlinePlayers.size > 1) defaultLocale else onlinePlayers.first().language()
    private val bar = BossBar.bossBar(cmp("Waiting for server..."), 1f, BossBar.Color.WHITE, BossBar.Overlay.PROGRESS)
    private val msgHeight = dominantLocale.msgString("event.forceHunt.height")
    private val msgItems = dominantLocale.msgString("event.forceHunt.item")
    private val msgBiomes = dominantLocale.msgString("event.forceHunt.biome")
    private val msgWaiting = dominantLocale.msgString("event.forceHunt.waiting")
