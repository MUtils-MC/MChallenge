...
import de.miraculixx.mchallenge.modules.challenges.*
...
class CollectBattle : Challenge {
    ...
    private val dominantLocale = getDominantLocale()
    private val itemKey = NamespacedKey(namespace, "collectbattle.indicator")
    private val msgFinished = cmp(dominantLocale.msgString("event.collectBattle.finished"), cSuccess)
    private val msgNotAllowed = cmp(dominantLocale.msgString("event.collectBattle.notAllowed"), cError)
    private val msgTimeLeft = dominantLocale.msgString("event.collectBattle.timeLeft")

    private val maxSetItemTime: Duration
    private val cooldownTime: Int
    private val bufferTime: Int
    private val cooldownBar: BossBar = BossBar.bossBar(dominantLocale.msg("event.collectBattle.cooldown"), 1f, BossBar.Color.WHITE, BossBar.Overlay.PROGRESS)
    ...
    private val onF = listen<PlayerSwapHandItemsEvent>(register = false) {
        ...
        if (item.handleTag()) {
            player.soundEnable()
            player.sendMessage(prefix + player.msg("event.collectBattle.register", listOf("<lang:${type.translationKey()}>")))
            ...
            when (state) {
                CollectBattleState.SETTING -> {
                    ...
                    if (activePlayer.isEmpty() || timer.time == ZERO) {
                        ...
                        players.forEachIndexed { index, uuid ->
                            ...
                            player?.sendMessage(prefix + player.msg("event.collectBattle.startHunt", listOf("<lang:${data.target.translationKey()}>", previousPlayerName, timeToGather.getFormatted())))
                ...
                CollectBattleState.HUNTING -> {
                    if (activePlayer.isEmpty()) {
                        ...
                        broadcast(prefix, "event.collectBattle.startCooldown", listOf(roundCounter.toString()))
                        startCooldown()
