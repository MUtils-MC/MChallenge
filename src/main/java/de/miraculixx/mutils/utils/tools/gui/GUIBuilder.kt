...
class GUIBuilder(private val player: Player, private val preset: GUI, animation: GUIAnimation = GUIAnimation.DEFAULT) {
    ...
    private fun createInventory(size: Int? = null, title: String? = null): Inventory {
        ...
        val t = title ?: preset.title
        return Bukkit.createInventory(null, s * 9, t)
    }
    ...
    fun open() {
        player.openInventory(finalInv ?: return)
    }
    ...
    fun storage(filterChange: StorageFilters?, import: Map<ItemStack, Boolean>? = null): GUIBuilder {
        ...
        val filter = if (filterChange == null) {
            ...
            if (preInv.size != 6 * 9) StorageFilters.NO_FILTER
            else {
                val meta = preInv.getItem(49)?.itemMeta
                if (meta == null || meta.lore == null || (meta.lore?.size ?: 0) < 5) StorageFilters.NO_FILTER
                else StorageFilters.valueOf(meta.lore!![4].removePrefix("§7∙ ").replace(' ', '_'))
            }
        } else filterChange
