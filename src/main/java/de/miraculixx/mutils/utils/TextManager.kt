...
fun getComponentList(key: String, inline: String? = null, input: String? = null): List<Component> {
    val config = ConfigManager.getConfig(Configs.LANGUAGE)
    val final = config.getStringList(key).map { mm.deserialize((inline ?: "") + it.replace("<INPUT>", input ?: "none")) }

    return final.ifEmpty { listOf(Component.text(key).color(NamedTextColor.RED)) }
}
