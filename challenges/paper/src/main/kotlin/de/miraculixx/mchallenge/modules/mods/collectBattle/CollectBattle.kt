    ...
    private fun startNewRound() {
        ...
        playablePlayers.forEach { player ->
            ...
            itemPresets[player] = TargetData(Material.AIR, InternalTimer(maxSetItemTime, {
                ...
                bar.name(cmp("$msgTimeLeft ") + cmp(duration.getFormatted(), cHighlight) + cmp(" (Press ") + Component.keybind("key.swapOffhand").color(cMark) + cmp(")"))
                    ...
                    if (activePlayer.isEmpty() || timer.time == ZERO) {
                        ...
                        players.forEachIndexed { index, uuid ->
                            ...
                            data.time = InternalTimer(timeToGather, {
                                ...
                                data.bar.name(cmp("$msgTimeLeft ") + cmp(duration.getFormatted(), cHighlight) + cmp(" - ") + Component.translatable(data.target.translationKey()).color(cMark))
